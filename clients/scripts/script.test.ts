...
describe('scripts', () => {
  ...
  test('set_wallet_provider', async () => {
    ...
    let resp1 = await deployer.admin_wallet_provider_add({
      wallet_provider: me_v1_canister_id,
  ...
  test('app_main_list', async () => {
    ...
    let resp1 = await deployer.app_main_list({
      query_param: { ByCategory: { category: { System: null } } },
    });
    console.log(resp1.Ok.apps);

    console.log(`\t\t list Vault app\n`);
    let resp2 = await deployer.app_main_list({
  ...
  test('get_log', async () => {
    ...
    let logs = await deployer.canister_log_get(
      BigInt((Date.now() - 1000 * 60 * 30) * 1000000),
      BigInt(Date.now() * 1000000),
    ); // use nanoseconds
    logs.forEach(log => {
      console.log(
        `canister_id: ${log.canister_id}, message: ${log.log}, ts: ${log.created_at}`,
      );
    });
  });

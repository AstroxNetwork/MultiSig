...
export class StoreConnection extends BaseConnection<_SERVICE> {
  ...
  async approve_app_version({
    version,
    app_id,
  }: ApproveAppVersionRequest): Promise<Result> {
  ...
  async list_user({ name }: ListUserRequest): Promise<Result_8> {
    const result = await this._actor.list_user({ name });
    return result;
  }

  async list_app(category: Category): Promise<Result_6> {
    const result = await this._actor.list_app({
      query_param: { ByCategory: { category } },
    });
    return result;
  }
  ...
  async create_app_order({ app_id }: CreateOrderRequest): Promise<Result_1> {
    const result = await this._actor.create_app_order({ app_id });
    return result;
  }

  
  async create_recharge_order({ amount }: CreateRechargeRequest): Promise<Result_1> {
    const result = await this._actor.create_recharge_order({ amount });
    return result;
  }
  ...
  async new_app_version({
    version,
    app_id,
  }: NewAppVersionRequest): Promise<Result_5> {
    const result = await this._actor.new_app_version({ version, app_id });
    return result;
  }

  async notify_payment({ memo }: { memo: bigint }): Promise<Result_5> {
  ...
  async register_app({
    ...
    category,
    price,
  }: RegisterAppRequest): Promise<Result_3> {
    const result = await this._actor.register_app({
      name,
      app_id,
      category,
      price,
  ...
  async set_role({
    ...
    is_app_developer,
    is_manager,
  }: SetRoleRequest): Promise<Result_10> {
    const result = await this._actor.set_role({
      user_id,
      is_app_auditer,
      is_app_developer,
      is_manager,
  ...
  async set_frontend_address({
    canister_id,
    app_id,
    version,
  }: SetFrontendAddressRequest): Promise<Result_5> {
    const result = await this._actor.set_frontend_address({
      canister_id,
      app_id,
      version,
    });

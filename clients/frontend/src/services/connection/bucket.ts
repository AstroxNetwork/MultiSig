import {
  ...
  AppFilesRequest,
  AppInfoRequest,
  AppNewVersionRequest,
  AppOperatorsResponse,
  LoadFileRequest,
  Permission,
  Result_1,
  Result_11,
...
export class BucketConnection extends BaseConnection<_SERVICE> {
  ...
  async add_managers(managers: Principal[]) {
    const result = await this._actor.add_managers({ managers });
    return result;
  }

  async app_create({ appid, caller }: AppCreateRequest): Promise<Result_1> {
  ...
  async app_info({ appid }: AppInfoRequest): Promise<Result_3> {
  ...
  async app_new_version({
    appid,
    fids,
    version,
  }: AppNewVersionRequest): Promise<Result_5> {
  ...
  async app_set_operators({
    appid,
    operators,
  }: AppOperatorsResponse): Promise<Result_7> {
  ...
  async upload_file({
    ...
    hash,
    version,
  }: UploadFileRequest): Promise<Result_13> {
    const result = await this._actor.upload_file({
      fid,
      appid,
      data,
      hash,

...
fn app_action_create(){
  ...
  let action = Service::app_action_create("path".to_string(), "to_address".to_string(), 1, extended, create_at);
...
fn action_sign_create_success(){
  ...
  wallet2.expect_action_main_invoke().returning(|path, to_address, amount_in_satoshi, _extended| {

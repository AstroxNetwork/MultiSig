...
use crate::web3::{api, error, rpc, BatchTransport, DuplexTransport, RequestId, Transport};
use futures::{
...
where
    ...
    B::Out: 'static + Send,
    ABatch:
        futures::Future<Output = error::Result<Vec<error::Result<rpc::Value>>>> + 'static + Send,
    BBatch:
        futures::Future<Output = error::Result<Vec<error::Result<rpc::Value>>>> + 'static + Send,
{

use crate::web3::{
    api::Namespace,
    contract::ens::{
        public_resolver::PublicResolver, registry::Registry, reverse_resolver::ReverseResolver,
    },
    signing::namehash,
...
impl<T: Transport> Ens<T> {
    ...
    pub async fn set_subdomain_owner(
        ...
        self.registry
            .set_subnode_owner(from, node, label, owner)
            .await
    }
    ...
    pub async fn set_record(
        ...
        self.registry
            .set_record(from, node, owner, resolver, ttl)
            .await
    }
    ...
    pub async fn set_approval_for_all(
        ...
        self.registry
            .set_approval_for_all(from, operator, approved)
            .await
    }
    ...
    pub async fn blockchain_address(
        &self,
        node: &str,
        coin_type: U256,
        ...
        if !resolver
            .check_interface_support(*BLOCKCHAIN_ADDR_INTERFACE_ID)
            .await?
        {
            return Err(ContractError::InterfaceUnsupported);
        }
    ...
    pub async fn set_blockchain_address(
        ...
        resolver
            .set_blockchain_address(from, node, coin_type, addr)
            .await
    }
    ...
    pub async fn pubkey(&self, node: &str) -> Result<([u8; 32], [u8; 32]), ContractError> {
        ...
        if !resolver
            .check_interface_support(*PUBKEY_INTERFACE_ID)
            .await?
        {
            return Err(ContractError::InterfaceUnsupported);
        }
    ...
    pub async fn content_hash(&self, node: &str) -> Result<Vec<u8>, ContractError> {
        ...
        if !resolver
            .check_interface_support(*CONTENTHASH_INTERFACE_ID)
            .await?
        {
            return Err(ContractError::InterfaceUnsupported);
        }

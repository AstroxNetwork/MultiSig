...
use crate::web3::{
    ...
    futures::Future,
    types::{
        AccessList, Address, BlockId, Bytes, CallRequest, FilterBuilder, TransactionCondition,
...
pub use crate::web3::contract::error::Error;
...
impl<T: Transport> Contract<T> {
    ...
    pub async fn call_with_confirmations(
        ...
        options: Options,
        confirmations: usize,
    ) -> crate::web3::error::Result<TransactionReceipt> {
        ...
        let fn_data = self
            ...
            .map_err(|err| crate::web3::error::Error::Decoder(format!("{:?}", err)))?;
...
mod contract_signing {
    ...
    impl<T: Transport> Contract<T> {
        pub async fn sign(
            ...
            chain_id: u64,
        ) -> crate::web3::Result<SignedTransaction> {
            let fn_data = self
                .abi
                .function(func)
                .and_then(|function| function.encode_input(&params.into_tokens()))
            ...
            accounts
                .sign_transaction(tx, from, key_name, derivation_path, chain_id)
                .await
        }
        ...
        pub async fn signed_call_with_confirmations(
            ...
            derivation_path: Vec<Vec<u8>>,
            chain_id: u64,
        ) -> crate::web3::Result<TransactionReceipt> {
            let poll_interval = time::Duration::from_secs(1);
            let signed = self
                .sign(
                    func,
...
mod tests {
    ...
    fn should_call_a_contract_function() {
        ...
        let result = {
            ...
            // when
            futures::executor::block_on(token.call(
                "name",
                (),
                Address::from_low_u64_be(5),
                Options::default(),
    ...
    fn should_estimate_gas_usage() {
        ...
        let result = {
            ...
            // when
            futures::executor::block_on(token.estimate_gas(
                "name",
                (),
                Address::from_low_u64_be(5),
                Options::default(),
